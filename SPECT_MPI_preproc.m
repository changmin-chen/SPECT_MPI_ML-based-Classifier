function S = SPECT_MPI_preproc(I)
...
c = zeros(block_sz(1), block_sz(2), 80);
c(:,:,1:20) = SA_rest;
c(:,:,21:30) = HLA_rest;
c(:,:,31:40) = VLA_rest;
c(:,:,41:60) = SA_stress;
...
SA_diff = c(:,:,1:20) - c(:,:,41:60);
HLA_diff = c(:,:,21:30) - c(:,:,61:70);
VLA_diff = c(:,:,31:40) - c(:,:,71:80);
c = cat(3, SA_diff, HLA_diff, VLA_diff);

%% Maximum intensity projection (MIP)
% ch1=SA MIP, ch2=HLA MIP, ch3=VLA MIP
S = zeros(size(c,1), size(c,2), 3);
S(:,:,1) = max(c(:,:,1:20),[],3); % SA MIP
S(:,:,2) = max(c(:,:,21:30),[],3); % HLA MIP
S(:,:,3) = max(c(:,:,31:40),[],3); % VLA MIP

%% Linear transformation of image value (for the purpose of saving .jpg file)
S = S*slope + intercept;
S = uint8(S);
